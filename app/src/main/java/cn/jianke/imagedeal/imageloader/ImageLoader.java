...
public class ImageLoader {
     ...
     * @param context 上下文
     * @param imageView 图片显示控件
     * @param customImageSizeModel 图片链接
     * @param defaultImage 默认占位图片
     * @return
     */
    public void load(Context context, ImageView imageView,
                     CustomImageSizeModel customImageSizeModel, Drawable defaultImage){
        try {
            load(context, imageView, customImageSizeModel, defaultImage, null, false);
        }catch (Exception ex){
        }
    }
...
//                .skipMemoryCache(true) //设置跳过内存缓存，但不保证一定不被缓存
     ...
     * @return
     */
    public void load(Context context, ImageView imageView, CustomImageSizeModel customImageSizeModel,
                     Drawable defaultImage, Drawable errorImage , boolean isCropCircle){
        // 图片加载库采用Glide框架
        DrawableTypeRequest request = Glide.with(context).load(customImageSizeModel);
        // 圆角裁切
        if (isCropCircle)
            request.bitmapTransform(new CropCircleTransformation(context));
        request.placeholder(defaultImage) //设置资源加载过程中的占位Drawable
    ...
    public ImageView resetImgWidthHeight(ImageView imageView, int originWidth, int originHeight,
        ...
        System.out.println("ddddddddddddddd targetWidth = " + targetWidth);
        System.out.println("ddddddddddddddd targetHeight = " + targetHeight);
